...
<svelte:head>
	<title>{t('Tags - Blog')}</title>
	<meta name="description" content={t('Browse all tags')} />
</svelte:head>

<div class="tags-page">
	<header class="page-header container container-narrow">
		<h1 class="page-title">Tags</h1>
		<p class="page-description">{t('Browse posts by tag')}</p>
	</header>

	<section class="tags-section container container-narrow">
		{#if tags.length === 0}
			<div class="empty-state">
				<p>{t('No tags yet. Add tags to your posts to see them here.')}</p>
			</div>
		{:else}
			<div class="tags-grid">
				{#each tags as tag}
