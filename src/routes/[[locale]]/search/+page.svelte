...
<svelte:head>
	...
	<meta name="description" content={t('Search posts')} />
</svelte:head>
...
<div class="search-page">
	...
	<section class="results-section container">
		{#if query}
			<div class="results-header">
				<p class="results-count">
					{#if results.length === 0}
						{t('No results found for "{0}"', query)}
					{:else}
						{t('Found {0} {1} for "{2}"', results.length, results.length === 1 ? 'result' : 'results', query)}
					{/if}
				</p>
			</div>
		...
		{:else}
			<div class="empty-state">
				<p>{t('Start typing to search posts.')}</p>
			</div>
		{/if}
	</section>
</div>
